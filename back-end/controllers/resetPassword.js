...
const forgotpassword = async (req, res) => {
  try {
    ...
    if (user) {
      const id = uuid.v4();
      const resetPasswordEntry = new ResetPassword({ id, userId: user._id });
      ...
      sendSmtpEmail.sender = { email: 'akash.puranik06@gmail.com', name: 'Dev' };  
...
const resetpassword = async (req, res) => {
  try {
    const { id } = req.params;
    const resetPasswordEntry = await ResetPassword.findOne({ id, isActive: true });

    if (resetPasswordEntry) {
      resetPasswordEntry.isActive = false;
      await resetPasswordEntry.save();
...
const updatepassword = async (req, res) => {
  try {
    ...
    const resetPasswordEntry = await ResetPassword.findOne({ id: resetpasswordid });

    if (resetPasswordEntry) {
      const user = await User.findById(resetPasswordEntry.userId);
